...
import psycopg2
conn = psycopg2.connect(dbname="cosmetik", host="localhost", user="student", password="root", port="5432")
cursor = conn.cursor()
...
def GetSubstanses(request):
    substanses = Substanses.objects.filter(sub_status=True).order_by('-sub_id')
...
def GetSubstanse(request, id):
    data_by_id = Substanses.objects.filter(sub_status=True).get(sub_id=id) # Берем один объект, находим его по id, которое получили в запросе
...
def delete_substanse(request, id):
    ...
    conn = psycopg2.connect(dbname="cosmetik", host="localhost", user="student", password="root", port="5432")
    cursor = conn.cursor()
    cursor.execute("UPDATE \"substanses\" SET sub_status=\'false\' WHERE sub_id = %s", [id]) # посылаем туда SQL запрос
    conn.commit()
    cursor.close()
    conn.close()
